...
To check whether two things are equal in Sigma, use the `?` symbol. This equality is strict. To check whether two variables have the same type, use the `??` symbol. To check if two numbers are within 5% of each other, use the `~` symbol (approximately equal). Use the `>` and `<` symbols for greater than and less than like in any other language. `>=` and `<=` also work as regular, but `≥` and `≤` are also supported by Sigma.
